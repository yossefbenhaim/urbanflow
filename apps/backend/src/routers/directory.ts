...
export const directoryRouter = router({
  getProviders: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      let q = ctx.supabase
        .from('profiles')
        .select('id, full_name, role, phone, provider_profiles(company, bio, service_types, operating_regions, years_experience), developer_profiles(company, bio, operating_regions, completed_projects)')
