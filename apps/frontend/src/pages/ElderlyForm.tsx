    ...
    <PageLayout>
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        ...
        <div className="bg-[#ebf1f7] border border-[#3b6b9c]/20 rounded-2xl p-5 mb-6 flex gap-3 items-start">
          ...
          <div>
            <h2 className="text-base font-bold text-[#1e3a5f] mb-1">זכויות מיוחדות לדיירים מעל גיל 70</h2>
        ...
        <div className="sc-card p-6">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h1 className="text-xl font-bold text-[#212121]">טופס קשיש / מוגבלות</h1>
              <p className="text-sm text-[#5a5a6e]">מילוי הטופס מבטיח שמירה על זכויותיך בפרויקט</p>
            </div>
          </div>
          ...
          <div className="space-y-5">
            {/* Age */}
            <div>
              <label className="block text-sm font-medium text-[#212121] mb-1">גיל</label>
              <input
                type="number"
                value={form.age}
              ...
              />
              {ageNum >= 70 && ageNum < 80 && (
                <p className="text-sm text-[#3b6b9c] mt-1 font-medium">✅ יש לך זכויות מיוחדות כקשיש בפרויקט</p>
              )}
              {ageNum >= 80 && (
                <p className="text-sm text-[#8b6f47] mt-1 font-medium">⚠️ מעל גיל 80 — היזם חייב להציג לך חלופות דיור (תיקון 6)</p>
              )}
            </div>
            ...
            <div className="border border-[#eeeeee] rounded-xl p-4">
              <label className="flex items-center gap-3 cursor-pointer">
                ...
                />
                <span className="text-sm font-medium text-[#212121]">יש לי מוגבלות</span>
              </label>
              {form.hasDisability && (
                <textarea
            ...
            {/* Accessibility Needs */}
            <div>
              <h3 className="text-sm font-bold text-[#212121] mb-3">צרכי נגישות</h3>
            ...
            {/* Preferred Area */}
            <div>
              <label className="block text-sm font-medium text-[#212121] mb-1">אזור מועדף למגורים</label>
              <input
                type="text"
            ...
            <div className="border border-[#eeeeee] rounded-xl p-4">
              <label className="flex items-center gap-3 cursor-pointer">
                <input
                  ...
                  className="w-5 h-5 rounded accent-sc-primary"
                />
                <span className="text-sm font-medium text-[#212121]">יש לי מלווה / אפוטרופוס</span>
              </label>
              {form.hasCompanion && (
                <div className="grid grid-cols-2 gap-3 mt-3">
                  <div>
                    <label className="block text-xs text-[#5a5a6e] mb-1">שם המלווה</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-xs text-[#5a5a6e] mb-1">טלפון המלווה</label>
                    <input
                      type="tel"
            ...
            {ageNum >= 70 && (
              <div>
                <h3 className="text-sm font-bold text-[#212121] mb-2">חלופות דיור מועדפות (לפי חוק)</h3>
                <p className="text-xs text-[#5a5a6e] mb-3">בחר/י את החלופות שמתאימות לך — היזם מחויב להציע אותן</p>
                <div className="space-y-2">
                  {LEGAL_ALTERNATIVES.map(alt => (
                    <label key={alt.value} className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-colors ${
                      form.legalAlternatives.includes(alt.value)
                        ? 'bg-[#3b6b9c]/10 border-[#3b6b9c]'
                        : 'bg-white border-[#eeeeee] hover:bg-[#f8f9fa]'
                    }`}>
                      <input
                        type="checkbox"
                        ...
                        className="w-5 h-5 rounded accent-sc-primary"
                      />
                      <span className="text-sm text-[#212121]">{alt.label}</span>
                    </label>
                  ))}
                </div>
              </div>
            ...
            {/* Notes */}
            <div>
              <label className="block text-sm font-medium text-[#212121] mb-1">הערות נוספות</label>
              <textarea
                value={form.notes}
          ...
          {save.isError && (
            <p className="text-red-500 text-sm mt-2 text-center">שגיאה בשמירה, נסה שנית</p>
          )}
        </div>
      </div>
    </PageLayout>
