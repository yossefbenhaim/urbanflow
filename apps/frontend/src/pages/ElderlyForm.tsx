  ...
  return (
    <div className="min-h-screen bg-sc-bg" dir="rtl">
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        {/* Info Banner */}
        <div className="bg-blue-50 border border-blue-200 rounded-2xl p-5 mb-6 flex gap-3 items-start">
          <span className="text-2xl flex-shrink-0">ℹ️</span>
          <div>
            <h2 className="text-base font-bold text-blue-900 mb-1">זכויות מיוחדות לדיירים מעל גיל 70</h2>
        ...
        <div className="sc-card p-6">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h1 className="text-xl font-bold text-sc-dark">טופס קשיש / מוגבלות</h1>
              <p className="text-sm text-sc-gray">מילוי הטופס מבטיח שמירה על זכויותיך בפרויקט</p>
            </div>
          </div>
          ...
          <div className="space-y-5">
            {/* Age */}
            <div>
              <label className="block text-sm font-medium text-sc-dark mb-1">גיל</label>
              <input
                type="number"
                value={form.age}
              ...
              />
              {ageNum >= 70 && ageNum < 80 && (
                <p className="text-sm text-blue-600 mt-1 font-medium">✅ יש לך זכויות מיוחדות כקשיש בפרויקט</p>
              )}
              {ageNum >= 80 && (
                <p className="text-sm text-orange-600 mt-1 font-medium">⚠️ מעל גיל 80 — היזם חייב להציג לך חלופות דיור (תיקון 6)</p>
              )}
            </div>

            {/* Disability */}
            <div className="border border-sc-gray-light rounded-xl p-4">
              <label className="flex items-center gap-3 cursor-pointer">
                <input
                  type="checkbox"
                  checked={form.hasDisability}
                  onChange={e => setForm(f => ({ ...f, hasDisability: e.target.checked }))}
                  className="w-5 h-5 rounded accent-sc-blue"
                />
                <span className="text-sm font-medium text-sc-dark">יש לי מוגבלות</span>
              </label>
              {form.hasDisability && (
                <textarea
            ...
            {/* Accessibility Needs */}
            <div>
              <h3 className="text-sm font-bold text-sc-dark mb-3">צרכי נגישות</h3>
              <div className="space-y-2">
                {[
                  ...
                  <label key={item.key} className="flex items-center gap-3 p-3 bg-white rounded-xl border border-sc-gray-light cursor-pointer hover:bg-sc-bg transition-colors">
                    <input
                      type="checkbox"
                      ...
                      className="w-5 h-5 rounded accent-sc-blue"
                    />
                    <span className="text-sm text-sc-dark">{item.label}</span>
                  </label>
                ))}
              </div>
            </div>
            ...
            {/* Preferred Area */}
            <div>
              <label className="block text-sm font-medium text-sc-dark mb-1">אזור מועדף למגורים</label>
              <input
                type="text"
            ...
            <div className="border border-sc-gray-light rounded-xl p-4">
              <label className="flex items-center gap-3 cursor-pointer">
                <input
                  ...
                  className="w-5 h-5 rounded accent-sc-blue"
                />
                <span className="text-sm font-medium text-sc-dark">יש לי מלווה / אפוטרופוס</span>
              </label>
              {form.hasCompanion && (
                <div className="grid grid-cols-2 gap-3 mt-3">
                  <div>
                    <label className="block text-xs text-sc-gray mb-1">שם המלווה</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-xs text-sc-gray mb-1">טלפון המלווה</label>
                    <input
                      type="tel"
            ...
            {ageNum >= 70 && (
              <div>
                <h3 className="text-sm font-bold text-sc-dark mb-2">חלופות דיור מועדפות (לפי חוק)</h3>
                <p className="text-xs text-sc-gray mb-3">בחר/י את החלופות שמתאימות לך — היזם מחויב להציע אותן</p>
                <div className="space-y-2">
                  {LEGAL_ALTERNATIVES.map(alt => (
                    <label key={alt.value} className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-colors ${
                      form.legalAlternatives.includes(alt.value)
                        ? 'bg-sc-blue/10 border-sc-blue'
                        : 'bg-white border-sc-gray-light hover:bg-sc-bg'
                    }`}>
                      <input
                        type="checkbox"
                        ...
                        className="w-5 h-5 rounded accent-sc-blue"
                      />
                      <span className="text-sm text-sc-dark">{alt.label}</span>
                    </label>
                  ))}
                </div>
              </div>
            ...
            {/* Notes */}
            <div>
              <label className="block text-sm font-medium text-sc-dark mb-1">הערות נוספות</label>
              <textarea
                value={form.notes}
