...
const STATUS_LABELS: Record<string, { label: string; cls: string }> = {
  pending: { label: '⏳ ממתין לאישור', cls: 'bg-sc-warning/15 text-sc-warning' },
  approved: { label: '✅ מאושר', cls: 'bg-sc-success/15 text-sc-success' },
  rejected: { label: '❌ נדחה', cls: 'bg-sc-error/15 text-sc-error' },
  expired: { label: '⌛ פג תוקף', cls: 'bg-sc-gray-light text-sc-gray' },
}
    ...
    <div className="min-h-screen bg-sc-bg" dir="rtl">
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-[22px] font-bold text-sc-dark mb-1">📋 ייפוי כוח</h1>
            <p className="text-sc-gray text-sm">ניהול ייפויי כוח לדירות שלך</p>
          </div>
        ...
        <div className="flex flex-col gap-3 mb-6">
          {(poas ?? []).map((poa: any) => (
            <div key={poa.id} className="sc-card p-5">
              <div className="flex items-start justify-between mb-3">
                <div>
                  <p className="text-sm font-bold text-sc-dark">
                ...
                <span className={`sc-badge text-xs ${STATUS_LABELS[poa.status]?.cls ?? 'bg-sc-gray-light text-sc-gray'}`}>
                  {STATUS_LABELS[poa.status]?.label ?? poa.status}
                </span>
              </div>
              <div className="flex gap-4 text-xs text-sc-gray">
              ...
              {poa.file_url && (
                <a href={poa.file_url} target="_blank" rel="noreferrer" className="text-xs text-sc-blue underline mt-2 inline-block">
                  📎 צפייה במסמך
                </a>
              )}
            </div>
          ...
          {(!poas || poas.length === 0) && !showForm && (
            <div className="sc-card p-8 text-center text-sc-gray">
        ...
        {showForm && (
          <div className="sc-card p-6">
            <h3 className="text-[17px] font-bold text-sc-dark mb-4">יצירת ייפוי כוח חדש</h3>
            <div className="flex flex-col gap-4">
              <div>
                <label className="block text-[13px] font-semibold text-sc-dark mb-1">מזהה מקבל ייפוי הכוח *</label>
              ...
              <div>
                <label className="block text-[13px] font-semibold text-sc-dark mb-2">סוג ייפוי כוח *</label>
                <div className="flex flex-col gap-2">
                  {POA_TYPES.map(t => (
                    <button key={t.value} type="button"
                      ...
                      <p className={`text-sm font-semibold ${form.poaType === t.value ? 'text-sc-blue' : 'text-sc-dark'}`}>{t.label}</p>
              ...
              <div>
                <label className="block text-[13px] font-semibold text-sc-dark mb-1">קישור למסמך נוטריוני</label>
              ...
              <div className="flex items-center gap-2">
                ...
                <label htmlFor="notarized" className="text-[13px] text-sc-dark">מסמך נוטריוני מאושר</label>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[13px] font-semibold text-sc-dark mb-1">תוקף מתאריך</label>
                  <input type="date" className="sc-input" value={form.validFrom}
                    onChange={e => setForm(f => ({ ...f, validFrom: e.target.value }))} />
                </div>
                <div>
                  <label className="block text-[13px] font-semibold text-sc-dark mb-1">תוקף עד</label>
