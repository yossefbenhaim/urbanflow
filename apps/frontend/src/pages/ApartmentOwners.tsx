    ...
    <div className="sc-card p-6" dir="rtl">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-[17px] font-bold text-[#212121] m-0">בעלי הדירה</h3>
            <p className="text-[13px] text-[#5a5a6e] mt-0.5">{owners?.length ?? 0} בעלים רשומים</p>
          </div>
        </div>
        ...
        {(owners ?? []).map((o: any) => (
          <div key={o.id} className="flex items-center gap-3 p-3 bg-[#f8f9fa] rounded-xl border border-[#eeeeee]">
            ...
            <div className="flex-1">
              <p className="text-sm font-semibold text-[#212121]">{o.user?.full_name ?? 'משתמש'}</p>
              <p className="text-xs text-[#5a5a6e]">
                {OWNERSHIP_TYPES.find(t => t.value === o.ownership_type)?.label ?? o.ownership_type} — {o.ownership_pct}%
              </p>
            </div>
            {o.has_proxy && (
              <span className="sc-badge bg-[#8b6f47]/15 text-[#8b6f47] text-xs">מיופה כוח</span>
            )}
            <span className={`sc-badge text-xs ${o.status === 'active' ? 'bg-[#4a8c5c]/15 text-[#4a8c5c]' : 'bg-sc-border text-[#5a5a6e]'}`}>
              {o.status === 'active' ? 'פעיל' : o.status}
            </span>
          </div>
        ))}
        {(!owners || owners.length === 0) && (
          <div className="text-center py-6 text-[#5a5a6e] text-sm">
      ...
      {/* Add Owner Form */}
      {showForm && (
        <div className="bg-[#f8f9fa] rounded-xl p-5 border border-[#3b6b9c]-light">
          <h4 className="text-[15px] font-bold text-[#212121] mb-4">הוספת בעלים חדש</h4>
          <div className="flex flex-col gap-3">
            <div>
              <label className="block text-[13px] font-semibold text-[#212121] mb-1">מזהה משתמש (UUID) *</label>
            ...
            <div>
              <label className="block text-[13px] font-semibold text-[#212121] mb-1">אחוז בעלות</label>
            ...
            <div className="flex items-center gap-2">
              ...
              <label htmlFor="hasProxy" className="text-[13px] text-[#212121]">יש מיופה כוח</label>
            </div>
            {form.hasProxy && (
              <div>
                <label className="block text-[13px] font-semibold text-[#212121] mb-1">מזהה מיופה כוח</label>
