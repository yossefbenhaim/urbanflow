    ...
    <div className="sc-card p-6" dir="rtl">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-[17px] font-bold text-sc-text m-0">בעלי הדירה</h3>
            <p className="text-[13px] text-sc-text-light mt-0.5">{owners?.length ?? 0} בעלים רשומים</p>
          </div>
        </div>
      ...
      <div className="flex flex-col gap-2.5 mb-4">
        {(owners ?? []).map((o: any) => (
          <div key={o.id} className="flex items-center gap-3 p-3 bg-sc-bg rounded-xl border border-sc-border">
            <div className="w-9 h-9 rounded-full bg-sc-light-blue flex items-center justify-center text-lg">
              {OWNERSHIP_TYPES.find(t => t.value === o.ownership_type)?.label?.split(' ')[0] ?? '👤'}
            </div>
            <div className="flex-1">
              <p className="text-sm font-semibold text-sc-text">{o.user?.full_name ?? 'משתמש'}</p>
            ...
            {o.has_proxy && (
              <span className="sc-badge bg-sc-gold-dark/15 text-sc-gold-dark text-xs">מיופה כוח</span>
            )}
            <span className={`sc-badge text-xs ${o.status === 'active' ? 'bg-sc-success/15 text-sc-success' : 'bg-sc-border text-sc-text-light'}`}>
              {o.status === 'active' ? 'פעיל' : o.status}
            </span>
          </div>
        ))}
        {(!owners || owners.length === 0) && (
          <div className="text-center py-6 text-sc-text-light text-sm">
      ...
      {/* Add Owner Form */}
      {showForm && (
        <div className="bg-sc-bg rounded-xl p-5 border border-sc-primary-light">
          <h4 className="text-[15px] font-bold text-sc-text mb-4">הוספת בעלים חדש</h4>
          <div className="flex flex-col gap-3">
            <div>
              <label className="block text-[13px] font-semibold text-sc-text mb-1">מזהה משתמש (UUID) *</label>
              <input className="sc-input" placeholder="מזהה המשתמש" value={form.userId}
                onChange={e => setForm(f => ({ ...f, userId: e.target.value }))} />
            </div>
            <div>
              <label className="block text-[13px] font-semibold text-sc-text mb-2">סוג בעלות *</label>
            ...
            <div>
              <label className="block text-[13px] font-semibold text-sc-text mb-1">אחוז בעלות</label>
            ...
            <div className="flex items-center gap-2">
              ...
              <label htmlFor="hasProxy" className="text-[13px] text-sc-text">יש מיופה כוח</label>
            </div>
            {form.hasProxy && (
              <div>
                <label className="block text-[13px] font-semibold text-sc-text mb-1">מזהה מיופה כוח</label>
